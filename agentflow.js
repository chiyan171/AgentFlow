...
if (command === 'docs-refresh') {
    refreshDocs(targetDir);
} else {
    console.log('用法: node agentflow.js docs-refresh [目录]');
}

function refreshDocs(target) {
    const docsDir = path.join(target, '.github/docs');
    const manifestPath = path.join(docsDir, 'manifest.json');
    const indexPath = path.join(docsDir, 'index.html');

    if (!fs.existsSync(docsDir)) {
        console.error(`❌ 文档目录不存在: ${docsDir}`);
        console.log(`💡 请先运行: agentflow init ${target}`);
        process.exit(1);
    }

    console.log('🔄 刷新文档门户（增量更新）...');
    console.log('');

    let existingManifest = { documents: [] };
    if (fs.existsSync(manifestPath)) {
        try {
            const content = fs.readFileSync(manifestPath, 'utf8');
            existingManifest = JSON.parse(content);
        } catch (e) {
            console.error('⚠️  无法读取现有 manifest.json，将重新创建');
        }
    }

    const existingDocs = new Map();
    if (existingManifest.documents) {
        existingManifest.documents.forEach(doc => {
    ...
    function processFile(filePath, type, defaultTitle) {
        const relativePath = path.relative(docsDir, filePath);
        const existingDoc = existingDocs.get(relativePath);
        ...
        if (existingDoc) {
            created = existingDoc.created || today;
    ...
    const changelogPath = path.join(docsDir, 'changelog', 'CHANGELOG.md');
    if (fs.existsSync(changelogPath)) {
        processFile(changelogPath, 'changelog', '变更日志');
    }
        ...
        if (fs.existsSync(dirPath)) {
            ...
            files.forEach(file => {
                if (file.endsWith('.md') && file !== 'README.md') {
                    ...
                    processFile(filePath, docType, null);
                }
            });
        }
    });
    ...
    console.log(`✅ 已更新 manifest.json (新增: ${addedCount}, 更新: ${modifiedCount})`);

    generateIndexHtml(indexPath, newManifest);
}

function generateIndexHtml(indexPath, manifest) {
...
<body>
    <div class="container">
            ...
            <div class="section">
                ...
                <ul class="doc-list" id="changelogList">
                    <li class="doc-item">
                        ...
                        <span class="badge badge-added">活跃</span>
                    </li>
                </ul>
            </div>
        ...
        <footer>
            ...
            <p>由 AgentFlow v1.0.0 自动管理</p>
        </footer>
    </div>
    
    <script>
        // 加载 manifest.json 更新页面
        fetch('manifest.json')
            .then(r => r.json())
            .then(data => {
